...
import { applyThemePalette, DEFAULT_THEME_ID, normalizeThemeId, readCachedThemeId, writeCachedThemeId } from "./theme";
import "./App.css";
...
const SORTABLE_COLUMN_SET = new Set(Object.keys(TABLE_SORT_OPTIONS));
const NULL_FILTER_VALUE = "__NULL_FILTER__";
const NULL_FILTER_LABEL = "Null";

function getSortMenuKey(columnKey) {
...
const DATE_PART_ORDER = ["day", "month", "year"];
...
function formatFilterTokens(tokens) {
  const formatted = tokens.map((token) => formatFilterTokenLabel(token)).filter(Boolean);
...
function withNullFilterOption(options) {
...
function matchChoiceTokens(rawValue, tokens, columnKey) {
  ...
  if (wantsNull && rowValues.some((value) => isBlankFilterValue(value))) return true;
  const normalizedRowValues = rowValues
    .map((value) => normalizeChoiceValue(value, columnKey))
    .filter((value) => value && value !== NULL_FILTER_VALUE);
  if (!normalizedRowValues.length) return false;
  const expectedTokens = normalizedTokens.filter((token) => token !== NULL_FILTER_VALUE);
  if (!expectedTokens.length) return false;
  return expectedTokens.some((token) => normalizedRowValues.includes(token));
}

function matchTextTokens(rawValue, tokens) {
  if (!tokens.length) return true;
  const normalizedTokenGroups = tokens
    .map((token) => splitSearchTerms(token))
    .filter((group) => group.length);
  if (!normalizedTokenGroups.length) return true;
  const rowValues = Array.isArray(rawValue) ? rawValue : [rawValue];
  return rowValues.some((value) => {
    if (value === undefined || value === null || value === "") return false;
    const normalizedValue = normalizeSearchText(value);
    if (!normalizedValue) return false;
    return normalizedTokenGroups.some((group) => group.every((term) => normalizedValue.includes(term)));
  });
}

function matchWordPrefixTokens(rawValue, tokens) {
  if (!tokens.length) return true;
  const normalizedTokenGroups = tokens
    .map((token) => splitSearchTerms(token))
    .filter((group) => group.length);
  ...
  return rowValues.some((value) => {
    ...
    return normalizedTokenGroups.some((group) =>
      group.every((term) => paddedValue.includes(` ${term}`))
    );
  });
}
...
function rowMatchesFilter(row, filter) {
  ...
  if (filter.key === "Name_Surname") {
    return matchWordPrefixTokens(rawValue, tokens);
  }
...
function parseSortableTextValue(row, columnKey) {
  const column = COLUMN_LOOKUP[columnKey];
  if (!column) return "";
  const rawValue = getColumnValue(row, column);
  return String(rawValue ?? "").trim();
}

function parseSortableDateValue(row) {
  const column = COLUMN_LOOKUP.Capitalization_Date;
  if (!column) return "";
  return normalizeRowDate(getColumnValue(row, column));
}

function compareSortStrings(a, b) {
...
function sortRows(rows, sortConfig) {
  ...
  if (!sortKey || direction === "none" || !SORTABLE_COLUMN_SET.has(sortKey)) return rows;
  ...
  sorted.sort((a, b) => {
    ...
    if (sortKey === "Capitalization_Date") {
      const aValue = parseSortableDateValue(a);
      const bValue = parseSortableDateValue(b);
      if (!aValue && !bValue) return 0;
      if (!aValue) return 1;
      if (!bValue) return -1;
      const result = compareSortStrings(aValue, bValue);
      return direction === "desc" ? -result : result;
    }

    const aValue = parseSortableTextValue(a, sortKey);
    const bValue = parseSortableTextValue(b, sortKey);
    if (!aValue && !bValue) return 0;
    if (!aValue) return 1;
    if (!bValue) return -1;
    const result = compareSortStrings(aValue, bValue);
...
function App() {
  ...
  const [themeColorId, setThemeColorId] = useState(DEFAULT_THEME_ID);
  const [themeDraftColorId, setThemeDraftColorId] = useState(DEFAULT_THEME_ID);
  const [themeDialogOpen, setThemeDialogOpen] = useState(false);
  const [loadingThemePreferences, setLoadingThemePreferences] = useState(false);
  const [savingThemePreferences, setSavingThemePreferences] = useState(false);
  ...
  const [tableSort, setTableSort] = useState({ key: null, direction: "none" });
  ...
  const effectiveThemeColorId = themeDialogOpen ? themeDraftColorId : themeColorId;
  const {
    canEditInventory,
    canEditCharts,
    canManageParameters,
  ...
  useEffect(() => {
    applyThemePalette(effectiveThemeColorId);
  }, [effectiveThemeColorId]);
  useEffect(() => {
    if (!isAuthenticated || !currentUser?.username) {
      setThemeColorId(DEFAULT_THEME_ID);
      setThemeDraftColorId(DEFAULT_THEME_ID);
      setLoadingThemePreferences(false);
      setThemePreferenceError("");
      return undefined;
    }

    const cachedThemeId = readCachedThemeId(currentUser.username);
    const initialThemeId = cachedThemeId || DEFAULT_THEME_ID;
    setThemeColorId(initialThemeId);
    setThemeDraftColorId(initialThemeId);

    let ignore = false;
    setLoadingThemePreferences(true);
    setThemePreferenceError("");

    api.get("/users/me/preferences")
      .then(({ data }) => {
        if (ignore) return;
        const nextThemeId = normalizeThemeId(data?.themeColor || DEFAULT_THEME_ID);
        setThemeColorId(nextThemeId);
        setThemeDraftColorId(nextThemeId);
        writeCachedThemeId(currentUser.username, nextThemeId);
      })
      .catch((error) => {
        if (ignore) return;
      ...
      .finally(() => {
        if (!ignore) setLoadingThemePreferences(false);
      });
  ...
  const openThemeSettings = useCallback(() => {
    ...
    setThemePreferenceError("");
    setThemeDraftColorId(themeColorId);
    setThemeDialogOpen(true);
  }, [isAuthenticated, themeColorId]);
  const closeThemeSettings = useCallback(() => {
    if (savingThemePreferences) return;
    setThemeDraftColorId(themeColorId);
    setThemePreferenceError("");
    setThemeDialogOpen(false);
  }, [savingThemePreferences, themeColorId]);
  const saveThemeSettings = useCallback(async () => {
    const selectedThemeId = normalizeThemeId(themeDraftColorId);
    setSavingThemePreferences(true);
    setThemePreferenceError("");
    try {
      const { data } = await api.put("/users/me/preferences", {
        themeColor: selectedThemeId,
      });
      const savedThemeId = normalizeThemeId(data?.themeColor || selectedThemeId);
      setThemeColorId(savedThemeId);
      setThemeDraftColorId(savedThemeId);
      if (currentUser?.username) writeCachedThemeId(currentUser.username, savedThemeId);
      setThemeDialogOpen(false);
    } catch (error) {
      setThemePreferenceError(
        getApiErrorMessage(error, "Theme settings could not be saved.")
      );
    } finally {
      setSavingThemePreferences(false);
    }
  }, [currentUser?.username, themeDraftColorId]);
  ...
  const handleInventoryViewSelect = useCallback((target) => {
    if (target !== "inventory" && target !== "deleted") return;
    if (activeNav !== target) handleNavigate(target);
    else setCurrentPage(1);
  }, [activeNav, handleNavigate]);

  const handleInventoryCountryClear = useCallback((target = "inventory") => {
    setFilters((prev) => {
  ...
  const handleInventoryCountrySelect = useCallback((target, value) => {
    if (!value) return;
  ...
  const useClientSorting = isTableViewActive && tableSort.direction !== "none" && Boolean(tableSort.key);
  ...
  const rowParams = useMemo(() => {
    ...
    preparedFilters.forEach((filter) => {
      ...
      if (useClientFiltering && !CHOICE_COLUMN_SET.has(filter.key)) {
        return;
      }
      if (paramKey) {
        if (CHOICE_COLUMN_SET.has(filter.key)) {
          ...
          params[paramKey] = normalizeSearchText(trimmed);
        }
        return;
      }
      ...
      if (!params.search) {
        ...
        if (accessor) {
          ...
          params.search = normalizeSearchText(trimmed);
        }
      }
    });
    return params;
  }, [useUnboundedLimit, limit, effectivePage, selectedCountry?.id, preparedFilters, activeNav, hasInventoryCountryFilters, useClientFiltering]);
  ...
  const handleTableSortSelect = useCallback((columnKey, nextValue) => {
    if (!columnKey || !SORTABLE_COLUMN_SET.has(columnKey) || !nextValue || nextValue === "none") {
      setTableSort({ key: null, direction: "none" });
      setOpenChoiceColumn(null);
      return;
    }
    setTableSort({ key: columnKey, direction: nextValue });
    setOpenChoiceColumn(null);
  }, []);
  ...
  const handleTableSortToggle = useCallback((columnKey, anchorEl) => {
    setOpenChoiceColumn((prev) => {
      const menuKey = getSortMenuKey(columnKey);
  ...
  useEffect(() => {
    ...
    api.get("/rows", { params: rowParams })
      .then(({ data }) => {
        ...
        if (useClientSorting) {
          processedItems = sortRows(processedItems, tableSort);
        }
              ...
              <div className="table-date-popover" role="dialog" aria-label="Capitalization date filter">
                {dateFilter.mode === "range" ? (
                  <>
                    <input
                      ...
                      type="text"
                      inputMode="numeric"
                      placeholder="From DD/MM/YY or YYYY-MM-DD"
                    ...
                    <input
                      ...
                      type="text"
                      inputMode="numeric"
                      placeholder="To DD/MM/YY or YYYY-MM-DD"
                      value={dateFilter.to}
                      onChange={(event) =>
                ...
                {dateFilter.mode === "on_or_before" ? (
                  <input
                    ...
                    type="text"
                    inputMode="numeric"
                    placeholder="Until DD/MM/YY or YYYY-MM-DD"
                    value={dateFilter.to}
                    onChange={(event) =>
                      setDateFilter((prev) => ({ ...prev, to: event.target.value }))
                    }
              ...
              <thead>
                <tr>
                  ...
                  {visibleColumns.map((column) => {
                    const isCountryColumn = column.key === "Country";
                    const isChoiceColumn = isCountryColumn || CHOICE_COLUMN_SET.has(column.key);
                    const isSortableColumn = SORTABLE_COLUMN_SET.has(column.key);
                    const sortMenuKey = getSortMenuKey(column.key);
                    const sortOptions = TABLE_SORT_OPTIONS[column.key] ?? [];
                    const currentSortDirection =
                      tableSort.key === column.key ? tableSort.direction : "none";
                    const optionList = isCountryColumn
                      ? inventoryCountryOptions
                      : (choiceFieldMap[column.key] ?? []);
                    const normalizedChoiceSearch = foldTr(choiceSearchQuery);
                    const hasChoiceSearch = Boolean(normalizedChoiceSearch);
                    const choiceOptions = withNullFilterOption((Array.isArray(optionList) ? optionList : [])
                      .map((item) => {
                    ...
                    const filteredChoiceOptions = hasChoiceSearch
                      ? choiceOptions.filter((option) => {
                          const searchTerms = splitSearchTerms(normalizedChoiceSearch);
                        ...
                        <div className="column-header-inner">
                          ...
                          {isSortableColumn ? (
                            <div className="choice-filter">
                              <button
                                type="button"
                                className={`choice-trigger${currentSortDirection !== "none" ? " is-active" : ""}`}
                                onClick={(event) => handleTableSortToggle(column.key, event.currentTarget)}
                              ...
                              {openChoiceColumn === sortMenuKey ? (
                                <div
                                  className="choice-menu"
                                  ref={(node) => {
                                    if (openChoiceColumn === sortMenuKey) {
                                ...
                                >
                                  <button
                                    type="button"
                                    className="choice-option choice-option--clear"
                                    onClick={() => handleTableSortSelect(column.key, "none")}
                                  >
                                    Clear sort
                                  </button>
                                  {sortOptions.filter((option) => option.value !== "none").map((option) => {
                                    const isActive = currentSortDirection === option.value;
                                    return (
                                      <button
                                        type="button"
                                        key={option.value}
                                        className={`choice-option${isActive ? " is-active" : ""}`}
                                        onClick={() => handleTableSortSelect(column.key, option.value)}
                                      >
                                        {option.label}
                                      </button>
                                    );
    ...
    <section className="map-section home-dashboard">
      <div className="home-grid">
        <div className="coverage-card coverage-card--map">
          ...
          <MiddleEastMap data={mapData} selectedCountry={selectedCountry} />
        </div>
        <SpareCoverageChart
          data={mapData}
  ...
  const inventoryContent = renderTableSection(null, { showCreate: false, showCountryChip: false });
  ...
  return (
    <div className="app-shell">
      <Header
        ...
        settingsDisabled={!isAuthenticated || loadingThemePreferences}
        canGoBack={canGoBack}
        ...
        onInventoryCountrySelect={handleInventoryCountrySelect}
      ...
      <ThemeSettingsDialog
        ...
        value={themeDraftColorId}
        saving={savingThemePreferences}
        error={themePreferenceError}
        onClose={closeThemeSettings}
        onChange={setThemeDraftColorId}
