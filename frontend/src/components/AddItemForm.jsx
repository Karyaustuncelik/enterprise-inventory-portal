...
const REQUIRED_FIELDS = ["Country", "Name_Surname", "Hardware_Serial_Number", "Asset_Number"];
...
export default function AddItemForm({
  ...
  const isLenovoAppManufacturer = useMemo(() => {
    const normalized = String(form.Hardware_Manufacturer ?? "")
      .trim()
      .replace(/\s+/g, " ")
      .toLowerCase();
    return normalized === "lenovo app";
  ...
  const computerNameEnabled = useMemo(() => {
    ...
    return isLenovoAppManufacturer || normalizedType === "laptop" || normalizedType === "desktop";
  }, [form.Hardware_Type, isLenovoAppManufacturer]);
  ...
  useEffect(() => {
    ...
    const useLenovoAppFormula = manufacturer === "lenovo app";

    if (useLenovoAppFormula) {
      if (!country || !serial) {
  ...
  const handleUserNameSelect = (result) => {
    ...
    setField("User_Name", result.username);
    setUserDirectoryResults([]);
    setUserDirectoryError("");
    setUserSearchActive(false);
  };
  ...
  return (
    <form className="add-form" onSubmit={handleSubmit}>
      ...
      <div className="add-grid add-grid--columns">
        <div className="add-grid-column add-grid-column--left">
          <div className="fg">
          ...
          <div className="fg required">
            ...
            <input
              ...
              onChange={(event) => setField("Name_Surname", event.target.value)}
              maxLength={50}
              required
            />
          </div>
        ...
        <div className="add-grid-column add-grid-column--right">
          <div className="fg required">
