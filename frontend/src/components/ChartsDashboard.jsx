...
function buildChartTitle(chart, columns) {
  ...
  const groupFilterLabel =
    chart.groupBy !== "none" && chart.groupFilterValue?.trim()
      ? ` (${chart.groupFilterValue.trim()})`
      : "";
  const filterLabel =
    chart.filterBy !== "none" && chart.filterValue?.trim()
      ? ` - ${getColumnLabel(columns, chart.filterBy)} = ${chart.filterValue.trim()}`
      : "";
  return `${metricPrefix} by ${groupLabel}${groupFilterLabel}${filterLabel}`;
}

function filterRowsForChart(rows, chart, columns) {
  ...
  const tokens = splitMultiFilterValues(chart.filterValue)
    .map((token) => normalizeFilterValue(token, chart.filterBy))
    .filter(Boolean);
  ...
  return rows.filter((row) => {
    ...
    return values.some((value) => {
      const normalized = normalizeFilterValue(value, chart.filterBy);
...
function buildChartBars(rows, chart, columns) {
  const filtered = filterRowsForChart(rows, chart, columns);
  ...
  const groupTokens = splitMultiFilterValues(chart.groupFilterValue)
    .map((token) => normalizeFilterValue(token, chart.groupBy))
    .filter(Boolean);
  ...
  filtered.forEach((row) => {
    ...
    values.forEach((value) => {
      ...
      const normalizedKey = normalizeFilterValue(label, chart.groupBy) || "unknown";
      if (groupTokens.length && !groupTokens.some((token) => normalizedKey.includes(token))) {
        return;
      }
...
function exportSvgToCanvas(svgEl) {
            ...
            {bars.map((bar, index) => {
              ...
              return (
                <g key={`${bar.label}-${index}`} transform={`translate(${x}, ${y})`}>
                  {labelText !== bar.label ? <title>{bar.label}</title> : null}
                  <rect width={baseBarWidth} height={height} rx="6" fill="#e1000f" />
                  <text
                    x={baseBarWidth / 2}
                    y={labelY}
                    textAnchor="end"
                    fontSize="9"
                    fill="rgba(31,35,40,0.7)"
...
export default function ChartsDashboard({ columns = [], canEdit = true }) {
  ...
  const groupValueOptions = useMemo(() => {
    ...
    for (const row of rows) {
      ...
      for (const value of values) {
        const label = String(value ?? "").trim() || "Unknown";
        const normalizedKey = normalizeFilterValue(label, chartDraft.groupBy) || "unknown";
  ...
  const chartCards = useMemo(() => {
    return charts.map((chart) => {
      ...
      const bars = buildChartBars(rows, chart, columns);
      const groupLabel =
        chart.groupBy === "none" ? "Total" : getColumnLabel(columns, chart.groupBy);
